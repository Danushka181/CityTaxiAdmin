...
const isActive = (route) => {
    const urlFistPart = page.url.split('/')[1];
    return route === '/' + urlFistPart;
}
...
<template>
    <aside
        class="absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col border-r overflow-y-hidden duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 -translate-x-full">
            ...
            <nav class="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
                <div>
                    ...
                    <ul class="mb-6 flex flex-col gap-1.5">
                        ...
                        <li>
                            <Link
                                :class="{ 'active': isActive('/passenger') }"
                                :href="route('passenger.index')"
                               class="router-link-active router-link-exact-active group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 bg-graydark dark:bg-meta-4">
                              <span>
                                <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                  <path
